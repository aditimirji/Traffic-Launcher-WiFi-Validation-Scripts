...
CONFIG = {
    ...
    "OUTPUT_NAME": "TAG_73_1_full_ml",
    "ENABLE_WL_COUNTERS": False,       # Set to True to enable wl counters calls, False to disable
    "ENABLE_DLS_TOGGLE": True,        # Set to True to enable periodic DLS toggle (queries channel, runs shmem, then dls -f to switch bands)
...
def dls_toggle_monitor(dut_ip, user, password, stop_event, interval=60, output_files=None, traffic_started_event=None):
    """Periodically runs DLS toggle commands on the DUT by alternating wl eht dls -f 0/1.
    
    Workflow:
    1. Waits `interval` seconds after traffic starts (collects avg for 'pre-toggle' segment)
    2. Fires 'wl eht dls -f 0' on first toggle, then alternates 0/1 every `interval` seconds
    3. After each toggle, waits `interval` seconds (collecting avg for 'post-toggle-N' segment)
    4. Prints one [DLS-AVG] line per segment window
    
    No wl status / wl shmem commands used - simple toggle only.
    ...
    # Persist file read positions across all _collect_and_print_avg calls so each
    # interval window only reads NEW bytes (0-10s window, 11-20s window, etc. are independent)
    file_positions = {fpath: 0 for fpath in (output_files or [])}

    def _collect_and_print_avg(seconds, seg_label):
        """Wait `seconds` collecting iperf samples from output files, then print one avg line."""
        samples = []
        # file_positions captured from outer scope - carries byte offset across calls
        # so each segment window only averages its own traffic, not cumulative data
        waited = 0
    ...
    # Wait for traffic to actually start before beginning the interval countdown
    if traffic_started_event is not None:
        log_print(f"[DLS] Waiting for traffic to be detected before starting {interval}s DLS interval countdown...")
        while not traffic_started_event.is_set():
            if stop_event.is_set() or FATAL_STOP_EVENT.is_set():
                log_print(f"[DLS] Stopped while waiting for traffic start")
                return
            time.sleep(0.5)
        ts_traffic = time.strftime('%Y-%m-%d %H:%M:%S')
        log_print(f"[DLS] [{ts_traffic}] Traffic detected - starting {interval}s interval before first DLS toggle")

    # Initial delay: wait `interval` seconds AFTER traffic starts, BEFORE first toggle
    log_print(f"[DLS] Waiting {interval} seconds for traffic to run before first DLS toggle...")
    _collect_and_print_avg(interval, "pre-toggle")
    ...
    # Determine starting toggle state based on current connection band:
    # - 5G connected -> first toggle = 0 (switch to 2G)
    # - 2G connected -> first toggle = 1 (switch to 5G)
    conn = os.path.basename(CONNECTION_SCRIPT).lower()
    if "2g" in conn:
        toggle_state = 1  # Currently 2G -> switch to 5G first
        log_print(f"[DLS] 2G connection detected - first toggle will be wl eht dls -f 1 (switch to 5G)")
    else:
        toggle_state = 0  # Currently 5G (default) -> switch to 2G first
        log_print(f"[DLS] 5G connection detected - first toggle will be wl eht dls -f 0 (switch to 2G)")
    segment_num = 1   # increments after each toggle fires

    while not stop_event.is_set() and not FATAL_STOP_EVENT.is_set():
        timestamp = time.strftime('%Y-%m-%d %H:%M:%S')

        dls_cmd = f"wl eht dls -f {toggle_state}"
        log_print(f"[DLS] [{timestamp}] Running command on DUT: {dls_cmd}")
        ...
        try:
            ...
            if result.returncode == 0:
                log_print(f"[DLS] [{timestamp_result}] Command successful: {dls_cmd}")
                if result.stdout:
                    log_print(f"[DLS] Output: {result.stdout[:200]}")
            else:
                log_print(f"[DLS] [{timestamp_result}] Command failed (exit code {result.returncode}): {dls_cmd}")
                if result.stderr:
                    log_print(f"[DLS] Error: {result.stderr[:200]}")
        except subprocess.TimeoutExpired:
            timestamp_timeout = time.strftime('%Y-%m-%d %H:%M:%S')
            log_print(f"[DLS] [{timestamp_timeout}] Command timed out: {dls_cmd}")
        except Exception as e:
            timestamp_error = time.strftime('%Y-%m-%d %H:%M:%S')
            log_print(f"[DLS] [{timestamp_error}] Error running command: {e}")

        # Toggle state for next iteration
        toggle_state = 1 - toggle_state

        # Wait for next toggle interval and collect avg
        _collect_and_print_avg(interval, f"post-toggle-{segment_num}")
        segment_num += 1
...
def handle_not_associated_reconnect_and_restart(dut_ip, user, password, term=None, term_arg=None, max_retries=2, filepath=None, mode_name=""):
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            # Log full DUT connection output to monitor logs
            if reconnect_result.stdout and reconnect_result.stdout.strip():
                log_print(f"[RECONNECT] --- {script_name} stdout ---")
                for ln in reconnect_result.stdout.splitlines():
                    log_print(f"[RECONNECT] {ln}")
                log_print(f"[RECONNECT] --- end stdout ---")
            if reconnect_result.stderr and reconnect_result.stderr.strip():
                log_print(f"[RECONNECT] --- {script_name} stderr ---")
                for ln in reconnect_result.stderr.splitlines():
                    log_print(f"[RECONNECT] {ln}")
                log_print(f"[RECONNECT] --- end stderr ---")
        except subprocess.TimeoutExpired:
...
def monitor_file_for_zero_throughput(filepath, dut_ip, user, password, stop_event, mode_name, ping_lock=None, term=None, term_arg=None, traffic_started_event=None):
    """
        ...
        with open(filepath, 'r') as f:
            ...
            while not stop_event.is_set():
                ...
                if re.search(r'\s+[1-9]\d*(?:\.\d+)?\s+[KMG]?bits/sec', line, re.IGNORECASE):
                    ...
                    ts_pass = time.strftime('%Y-%m-%d %H:%M:%S')
                    log_print(f"[Monitor] [{ts_pass}] Startup check PASSED - traffic detected at {startup_elapsed:.1f}s ({mode_name})")
                    # Signal DLS toggle monitor that traffic has started
                    if traffic_started_event is not None:
                        traffic_started_event.set()
                    break
            
            # If no traffic detected in 20 seconds, EXIT THE ENTIRE SCRIPT
            if not startup_traffic_detected and not stop_event.is_set():
                ts_fail = time.strftime('%Y-%m-%d %H:%M:%S')
                log_print(f"[Monitor] [{ts_fail}] STARTUP CHECK FAILED - No traffic detected after 20 seconds ({mode_name})")
            ...
            ts_nm = time.strftime('%Y-%m-%d %H:%M:%S')
            log_print(f"[Monitor] [{ts_nm}] Starting normal monitoring phase ({mode_name})")
            ...
            while not stop_event.is_set():
                ...
                if re.search(r'\s+0+(?:\.0+)?\s+[KMG]?bits/sec', line, re.IGNORECASE):
                    ...
                    ts_zero = time.strftime('%Y-%m-%d %H:%M:%S')
                    log_print(f"[Monitor] [{ts_zero}] Zero throughput detected (count: {zero_count}, consecutive: {consecutive_zeros}, elapsed: {elapsed:.1f}s) in {mode_name}")
                    log_print(f"[Monitor] [{ts_zero}] Line: {line.strip()}")
                    
                    # Zero stall observed - log it and reset counters, test continues until timer expires
                    if elapsed >= 1:
                        ts_stall = time.strftime('%Y-%m-%d %H:%M:%S')
                        log_print(f"[Monitor] [{ts_stall}] Zero stall ({elapsed:.1f}s) in {mode_name} - logged only, test continues")
                        zero_start_time = None
                        consecutive_zeros = 0
                else:
                    # Non-zero throughput detected
                    if consecutive_zeros > 0:
                        ts_nz = time.strftime('%Y-%m-%d %H:%M:%S')
                        log_print(f"[Monitor] [{ts_nz}] Non-zero throughput detected - resetting zero counter")
                        consecutive_zeros = 0
                        zero_start_time = None
    ...
    log_print(f"[Monitor] [{ts_end}] Stopped monitoring {filepath}. Total zero detections: {zero_count}, wl ver runs: {wl_ver_count}, wl status runs: {wl_status_count}, wl command timeouts: {timeout_count}")
...
def run_fresh_connection(dut_ip, user, password, mode_name):
    ...
    try:
        ...
        if result.stdout and result.stdout.strip():
            log_print(f"[CONNECT] --- {script_name} stdout ---")
            for ln in result.stdout.splitlines():
                log_print(f"[CONNECT] {ln}")
            log_print(f"[CONNECT] --- end stdout ---")
        if result.stderr and result.stderr.strip():
            log_print(f"[CONNECT] --- {script_name} stderr ---")
            for ln in result.stderr.splitlines():
                log_print(f"[CONNECT] {ln}")
            log_print(f"[CONNECT] --- end stderr ---")
        if result.returncode == 0:
...
def main():
    ...
    while idx < len(runs):
        ...
        if idx == 1:
            # First mode: wrap already ran the connection script before launching Python.
            # Just verify association with wl status and log it.
            ts = time.strftime('%Y-%m-%d %H:%M:%S')
            log_print(f"\n[INIT] [{ts}] Wrap already connected - running wl status to verify association")
            try:
                ...
                wl_out = status_result.stdout + status_result.stderr
                ts2 = time.strftime('%Y-%m-%d %H:%M:%S')
                log_print(f"[INIT] [{ts2}] wl status output:")
                for wl_line in wl_out.strip().splitlines():
                    log_print(f"[INIT]   {wl_line}")
                if "NOT ASSOCIATED" in wl_out.upper():
                    log_print(f"[INIT] [{ts2}] WARNING: DUT is NOT ASSOCIATED at start of first mode")
                else:
                    log_print(f"[INIT] [{ts2}] DUT is ASSOCIATED - proceeding with traffic")
            except Exception as e:
                log_print(f"[INIT] Error running wl status: {e}")
        else:
            # Between modes: run fresh connection using the same script wrap used
            run_fresh_connection(dut, user, password, cur_mode)

        # Pre-traffic diagnostics disabled in no_wl mode
        ...
        if server_on_remote:
            ...
            # Shared event: set by first monitor that detects traffic; DLS waits on it
            traffic_started_event = threading.Event()
            ...
            server_monitor = threading.Thread(
                ...
                kwargs={"traffic_started_event": traffic_started_event},
                daemon=True
            )
            ...
            if CONFIG.get("ENABLE_DLS_TOGGLE", False):
                dls_thread = threading.Thread(
                    ...
                    kwargs={"output_files": [server_out], "traffic_started_event": traffic_started_event},
                    daemon=True
                )
                dls_thread.start()
                log_print(f"[DLS] DLS toggle monitor thread started for {cur_mode}")
        else:
            # Local server
            header = f"echo ''; echo '========================================'; echo 'MODE: {cur_mode.upper()} - RUN {idx}/{len(runs)}'; echo 'Time: '$(date); echo '========================================'; echo ''"
